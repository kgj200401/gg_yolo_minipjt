"""Experiment E: A's data and settings, changing only initial learning rate."""
import csv
import json
import subprocess
import sys
...
RUN = ROOT/'runs/detect/ozm_lr0005'
OUT = ROOT/'runs/validation_lr0005'
REF = ROOT/'runs/reference_lr0005'
DOC = ROOT/'docs/파인튜닝 진행.md'


def update(result):
    text = DOC.read_text(encoding='utf-8')
    start, end = '<!-- E_RESULT_START -->', '<!-- E_RESULT_END -->'
...
def main():
    ...
    try:
        ...
        subprocess.run([sys.executable, '-u', 'experiments/train_model.py', '--data', 'dataset_augmented/data.yaml',
                        ...
                        '--lr0', '0.0005', '--optimizer', 'AdamW', '--patience', '25', '--workers', '2',
                        '--cos-lr', '--degrees', '15', '--shear', '10', '--fliplr', '0.5',
                        '--name', 'ozm_lr0005'], cwd=ROOT, check=True)
        ...
        assert differences['lr0'] == [0.001,0.0005]
        ...
        for conf in [.65,.60]:
            ...
            lines += ['',f'TP/FP/FN: A {a["tp"]}/{a["fp"]}/{a["fn"]} → E {b["tp"]}/{b["fp"]}/{b["fn"]}', '']
        a,b = fixed(reference,.65),fixed(current,.65)
        lines += ['| 클래스(65%) | A 정밀도 | E 정밀도 | A Recall | E Recall |','|---|---:|---:|---:|---:|']
        ...
        update(result)
        print('Experiment E complete.',flush=True)
    except Exception as exc:
        update(f'상태: 실행 실패 ({type(exc).__name__}). runs/logs/lr0005_training.log 및 .err.log 확인.')
        raise
