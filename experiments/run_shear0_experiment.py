"""Experiment H: A's data and settings, disabling shear augmentation only."""
import csv
import json
import subprocess
import sys
...
RUN = ROOT/'runs/detect/ozm_shear0'
OUT = ROOT/'runs/validation_shear0'
REF = ROOT/'runs/reference_shear0'
DOC = ROOT/'docs/파인튜닝 진행.md'


def update(result):
    text = DOC.read_text(encoding='utf-8')
    start, end = '<!-- H_RESULT_START -->', '<!-- H_RESULT_END -->'
...
def main():
    ...
    try:
        ...
        update(f'상태: 학습 진행 중. 시작: {started}\nA와 같은 데이터에서 shear만 10 → 0으로 변경. batch 16, lr0 0.001 유지.')
        subprocess.run([sys.executable, '-u', 'experiments/train_model.py', '--data', 'dataset_augmented/data.yaml',
                        ...
                        '--cos-lr', '--degrees', '15', '--shear', '0', '--fliplr', '0.5',
                        '--name', 'ozm_shear0'], cwd=ROOT, check=True)
        ...
        assert set(differences) <= {'shear','name','save_dir'}, differences
        assert differences['shear'] == [10.0,0.0]
        ...
        lines = ['### H 결과 — A 기준 shear 제거 실험 완료', '',
        ...
        for conf in [.65,.60]:
            ...
            lines += ['',f'TP/FP/FN: A {a["tp"]}/{a["fp"]}/{a["fn"]} → H {b["tp"]}/{b["fp"]}/{b["fn"]}', '']
        a,b = fixed(reference,.65),fixed(current,.65)
        lines += ['| 클래스(65%) | A 정밀도 | H 정밀도 | A Recall | H Recall |','|---|---:|---:|---:|---:|']
        ...
        update(result)
        print('Experiment H complete.',flush=True)
    except Exception as exc:
        update(f'상태: 실행 실패 ({type(exc).__name__}). runs/logs/shear0_training.log 및 .err.log 확인.')
        raise
