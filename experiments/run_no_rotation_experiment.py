"""Experiment B: train without 90-degree copies, validate, update the progress document."""
import csv
import json
import subprocess
import sys
...
def update_document(result):
    ...
    start, end = "<!-- B_RESULT_START -->", "<!-- B_RESULT_END -->"
...
def main():
    ...
    try:
        ...
        lines += ["", f"TP/FP/FN: A {a['tp']}/{a['fp']}/{a['fn']} → B {b['tp']}/{b['fp']}/{b['fn']}",
                  "", "**각 모델에서 정밀도 90% 이상 중 최고 Recall 조건**", "",
                  "| 모델 | 신뢰도 | Precision | Recall | F1 |", "|---|---:|---:|---:|---:|"]
        for label, report in (("A", baseline), ("B", current)):
        ...
        lines += ["", "**B 신뢰도별 수치**", "", "| 신뢰도 | Precision | Recall | F1 |", "|---|---:|---:|---:|"]
