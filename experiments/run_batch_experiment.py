"""Experiment G: A's data and settings, changing physical batch size to 8."""
import csv
import json
import subprocess
import sys
...
RUN = ROOT/'runs/detect/ozm_batch8'
OUT = ROOT/'runs/validation_batch8'
REF = ROOT/'runs/reference_batch8'
DOC = ROOT/'docs/파인튜닝 진행.md'


def update(result):
    text = DOC.read_text(encoding='utf-8')
    start, end = '<!-- G_RESULT_START -->', '<!-- G_RESULT_END -->'
...
def main():
    ...
    try:
        ...
        assert set(differences) <= {'batch','name','save_dir'}, differences
        assert differences['batch'] == [16,8]
        ...
        for conf in [.65,.60]:
            ...
            lines += ['',f'TP/FP/FN: A {a["tp"]}/{a["fp"]}/{a["fn"]} → G {b["tp"]}/{b["fp"]}/{b["fn"]}', '']
        a,b = fixed(reference,.65),fixed(current,.65)
        lines += ['| 클래스(65%) | A 정밀도 | G 정밀도 | A Recall | G Recall |','|---|---:|---:|---:|---:|']
        ...
        update(result)
        print('Experiment G complete.',flush=True)
    except Exception as exc:
        update(f'상태: 실행 실패 ({type(exc).__name__}). runs/logs/batch8_training.log 및 .err.log 확인.')
        raise
