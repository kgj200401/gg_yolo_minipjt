...
def main():
    ...
    try:
        subprocess.run([sys.executable, "-u", "train_model.py", "--data", "dataset_augmented/data.yaml",
        ...
        update_document(f"상태: 학습 또는 평가 실패 ({type(exc).__name__}). 로그 `runs/shear3_training.log`를 확인한다.\n시작: {started}")
        raise
